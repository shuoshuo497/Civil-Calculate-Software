...
class DrawingWidget(QtWidgets.QMainWindow):
    ...
    def setupLeftPane(self):
        ...
        forceDiagramButton.clicked.connect(self.exportDataToTxt)
    ...
    def exportDataToTxt(self):
        # 计算杆件数和节点数
        num_segments = len(self.active_numbers)
        num_nodes = len(self.active_numbers_picture)
        ...
        for item in self.scene.items():
            if isinstance(item, LineItem):
                ...
                segment_lines.append(f"{start_node_number} {end_node_number} {item.props['旋转角度']} {item.props['长度']}")
        ...
        for item in self.scene.items():
            if isinstance(item, PictureItem):
                node_lines.append(f"{displacement_x} {displacement_y} {item.props['受到x方向集中荷载']} {item.props['受到y方向集中荷载']}")
        ...
        subprocess.run(['python', 'C:\\Users\\li_ju\\Desktop\\homework\\Civil-Calculate-Software\\计算程序.py'])
